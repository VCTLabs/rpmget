...
normative: false
ref: ''
reviewed: tNgDUQchx6nvDWRoxKrGM55dM9-MbuDTTZtWeOyEhvE=
---
...
- support repo creation from both flat and tree layouts
  - includes inplace repo if `repo_dir` equals `top_dir`

Constraints:

- the createrepo option requires one of the following two allowed config rules
  - `top_dir` and `repo_dir` must be *different* paths when layout is tree
  - `top_dir` and `repo_dir` must be *the same* path when layout is flat
